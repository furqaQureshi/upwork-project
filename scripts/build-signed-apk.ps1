...
'@ | Set-Content -Path $tempPhp -Encoding ASCII
    ...
    & .\gradlew.bat 'assembleRelease' '--no-daemon' "-Pandroid.injected.signing.store.file=$storeFile" "-Pandroid.injected.signing.store.password=$storePass" "-Pandroid.injected.signing.store.type=$storeType" "-Pandroid.injected.signing.key.alias=$alias" "-Pandroid.injected.signing.key.password=$keyPass"
    ...
    Write-Host "APK generated: $($artifact.FullName)"
