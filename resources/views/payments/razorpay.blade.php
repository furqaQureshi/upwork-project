<x-app-layout>
    ...
    <form id="razorpay-callback" method="POST" action="{{ route('payments.callback.razorpay') }}" class="hidden">
        @csrf
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const options = {
                ...
                amount: @json((int) round(((float) $payment->amount) * 100)),
                currency: @json($payment->currency),
                name: @json(config('app.name')),
                description: @json('Featured ad for '.$listing->title),
                ...
                modal: {
                    ondismiss: function () {
                        window.location.href = @json(route('payments.checkout', $listing));
                    }
                },
